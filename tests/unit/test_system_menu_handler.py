...
@pytest.mark.asyncio
async def test_can_use_system_allows_developer_role():
    permissions = SimpleNamespace(
        is_supreme_admin=lambda *_args, **_kwargs: False,
        is_dev_admin=lambda *_args, **_kwargs: False,
        get_effective_role=AsyncMock(return_value="developer"),
    )
    handler = SystemMenuHandler(db_manager=SimpleNamespace(), permissions=permissions)

    allowed = await handler._can_use_system(11, "devuser")
